...
def gen_status_keyboard(status):
    return [InlineKeyboardButton("Init" if status != "0" else "[ Init ]", callback_data='ll_status_0'),
            InlineKeyboardButton("Cancel" if status != "1" else "[ Cancel ]", callback_data='ll_status_1'),
            InlineKeyboardButton("Over" if status != "2" else "[ Over ]", callback_data='ll_status_2'),
            InlineKeyboardButton("Receive" if status != "3" else "[ Receive ]", callback_data='ll_status_3')]
...
def page(from_user, status, current_page):
    ...
    keyboard_list = []
    keyboard_status = gen_status_keyboard(status)
